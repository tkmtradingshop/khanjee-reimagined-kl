...
const RecommendedDishes = ({
  ...
  return <section className="bg-gradient-to-b from-emerald-deep/5 to-background py-16">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {recommendedDishes.map(dish => <div key={dish.id} className="bg-card rounded-2xl overflow-hidden border border-border/50 shadow-soft hover:shadow-elegant transition-all duration-300 hover:-translate-y-2 group">
              {/* Image Container */}
              <div className="relative overflow-hidden">
                <img src={dish.image} alt={dish.name} className="w-full h-48 object-cover group-hover:scale-110 transition-transform duration-500" />
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {/* Quantity Selector */}
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQuantity(dish.id, (quantities[dish.id] || 1) - 1)} className="w-8 h-8 rounded-full bg-muted hover:bg-muted-foreground/20 flex items-center justify-center text-foreground transition-colors" disabled={(quantities[dish.id] || 1) <= 1}>
                      -
                    </button>
                    <span className="w-8 text-center font-medium text-foreground">
                      {quantities[dish.id] || 1}
                    </span>
                    <button onClick={() => updateQuantity(dish.id, (quantities[dish.id] || 1) + 1)} className="w-8 h-8 rounded-full bg-muted hover:bg-muted-foreground/20 flex items-center justify-center text-foreground transition-colors">
                      +
                    </button>
                  </div>
                </div>
                ...
                {/* Add to Cart Button */}
                <Button onClick={() => handleAddToCart(dish)} className="w-full bg-gradient-to-r from-brass-gold to-brass-light hover:from-brass-light hover:to-brass-gold text-emerald-deep font-semibold transition-all duration-300 hover:scale-105 shadow-glow">
